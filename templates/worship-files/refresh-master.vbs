...
For i = ppt.Presentations.Count To 1 Step -1
    If LCase(ppt.Presentations(i).FullName) = LCase(masterPath) Then
        ppt.Presentations(i).Saved = msoTrue
...
If Err.Number <> 0 Then
    ...
    On Error GoTo 0
    ppt.Quit
    Fail ChrW(&H7121) & ChrW(&H6CD5) & ChrW(&H95DC) & ChrW(&H9589) & ChrW(&H76EE) & ChrW(&H524D) & ChrW(&H958B) & ChrW(&H555F) & ChrW(&H7684) & " master.pptx" & ChrW(&H3002) & vbCrLf & msg
End If
On Error GoTo 0
...
Function BuildLinksFromDuplicate(ByVal slideObject, ByVal totalFiles, ByRef fileList, ByRef errorMessage)
    ...
    Dim currentX, currentY, textWidth, textHeight, lineHeight, gapWidth
    ...
    For i = 0 To totalFiles - 1
        ...
        If currentY + textHeight > containerBottom Then
            errorMessage = "AUTO_LINK_CONTAINER " & ChrW(&H7684) & ChrW(&H9AD8) & ChrW(&H5EA6) & ChrW(&H4E0D) & ChrW(&H5920) & ChrW(&H5BB9) & ChrW(&H7D0D) & ChrW(&H5168) & ChrW(&H90E8) & " " & totalFiles & " " & ChrW(&H500B) & " PPTX " & _
                ChrW(&H9023) & ChrW(&H7D50) & ChrW(&H3002) & vbCrLf & vbCrLf & _
                           ChrW(&H8ACB) & ChrW(&H589E) & ChrW(&H52A0) & ChrW(&H5B83) & ChrW(&H7684) & ChrW(&H9AD8) & ChrW(&H5EA6) & ChrW(&HFF0C) & ChrW(&H6216) & ChrW(&H8ABF) & ChrW(&H5C0F) & " TextBox 6 " & ChrW(&H7684) & ChrW(&H5B57) & _
